...
- [<span class="toc-section-number">2</span> Classes](#classes)
  - [<span class="toc-section-number">2.1</span>
    k1OrderLTIsysSiso](#k1orderltisyssiso)
    - [<span class="toc-section-number">2.1.1</span> Continuous
...
This package contains simulators of 1st and 2nd order LTI systems, for
applications in either continuous or discrete time. 

In particular with the 2nd order system simulators, one can use it to estimate
the derivative of stochastic signals. The 2-order filters allow to limit (up
and down) both the output and slopes. The N-order filter allow to estimate the
derivative of any order fo the input signal.
...
#### to create a SISO continuous 1st-order LTI system:
```python
    pole = -2.0
    y0   =  3.0
...
#### to create a SISO discrete 1st-order LTI system:
```python
    pole = -2.0
    y0   =  3.0
